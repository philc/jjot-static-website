...
// Note(philc): Patched this, because it ceased to work on firefox 3.6.
// else if (document.getBoxObjectFor != null) window.gecko = true;
else if (window.navigator.userAgent.toString().toLowerCase().indexOf("firefox") >= 0) window.gecko = true;
	...
	create: function(options){
		...
		return function(event){
			...
			var returns = function(){
				return fn.apply( (options.bind || fn), args);	//philc
...
Element.extend = function(properties){
	for (var property in properties){
    // NOTE(philc): firefox 4 doesn't like us doing this when property == "hidden".
    try { HTMLElement.prototype[property] = properties[property]; } catch(error) { }
...
Drag.Base = new Class({
	...
	start: function(event){
		...
		if (this.options.editor){	//philc
			rte.Events.addEvent("mouseup",this.bound.stop);
		}
		this.fireEvent('onStart', this.element);
		event.stop();
	},
	...
	check: function(event){
		...
		if (distance > this.options.snap){
			...
			if (this.options.editor)	// philc
				rte.Events.addEvent("mousemove",this.bound.drag);
	...
	drag: function(event){
		...
		/* if this mouseover event was relative to an iframe, adjust the mouse coords so they're relative to the page.
		 *  Note that the relative iframe will never be the iframe that we're currently dragging around -- just iframes
		 *  that we're dragging our note on top of. See top of ui.dd.js for more details -philc */
		if (event.event.relativeTo && event.event.relativeTo){	
			var el = $(event.event.relativeTo);
			var pos = el.getPosition([$('note-scroll')]);
			// make the mouse coordinates relative to the iframe
			this.mouse.now.x+=pos.x;
			this.mouse.now.y+=pos.y;
		}
	...
	// philc modifications: added "firecomplete" bool. This is so base classes (Drag.Move) can end a drag
	// and call the onComplete event themselves
	stop: function(fireComplete){
		document.removeListener('mousemove', this.bound.check);
		document.removeListener('mousemove', this.bound.drag);
		document.removeListener('mouseup', this.bound.stop);	
		
		if (this.options.editor){	// philc
			rte.Events.removeEvent('mousemove',this.bound.drag);
			// rte.Events.removeEvent('mousemove',this.bound.check);
			rte.Events.removeEvent("mouseup",this.bound.stop);
		}
		
		if (fireComplete!=false)	// philc
...
Drag.Move = Drag.Base.extend({
	...
	checkAgainst: function(el){
		if (el==this.element) return false;	//philc - don't allow us to drag over ourselves.
		
		// philc: let each droppable have its own overflow. Notes are in an overflown container, noteboards aren't.
		// el = el.getCoordinates(this.options.overflown);
		el = el.getCoordinates(el.overflow);
